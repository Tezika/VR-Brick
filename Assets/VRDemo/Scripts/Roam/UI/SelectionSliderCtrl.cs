...
namespace VRDemo.Game.Roam.UI{
	public class SelectionSliderCtrl : MonoBehaviour {
		public event BarFilledIEnumerator onBarFilledIenumerator;

		[SerializeField]private float _gazeTime = 2.0f;                     // how long need player to gaze the slider
		...
		[SerializeField] private VRInteractiveItem m_InteractiveItem;       // Reference to the VRInteractiveItem to determine when to fill the bar.                    // Reference to the VRInput to detect button presses.
		...
		private bool _isFilling = false;                                     //Whether the bar is filling?
		private bool _isCanFill  = true;
		...
		private IEnumerator FillBar ()
		{
			...
			while (m_Timer < fillTime)
			{
				...
				this._isCanFill = !this._isCanFill;
				yield break;
			}
		...
		private void HandleOver ()
		{
			...
			if (!this._isFilling && this._isCanFill) {
				this._isCanFill = false;
				StartCoroutine (gazeOverSlider());
			}
		}
		...
		IEnumerator gazeOverSlider(){
			yield return new WaitForSeconds (this._gazeTime);
			if (m_GazeOver) {
